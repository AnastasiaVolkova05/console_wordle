...
from words_russian import all_words as russian_words
import frases as fr
...
def color_string_letter(letter, color, string: list):
    '''This function color the element from the list.
       Keys:
...
class GameMenu():
    '''This class contains some frases,
    that you need to display during the game.
    This class hasn't got any methods'''
    def __init__(self, wordlist, instraction, welcome_frase, detail, alphabet,
                 play, word, lose, win, error):
...
class ThisGame(GameMenu):
    '''This class contains some game constants
    and also the correct and answer word for each game'''
...
def main():
    while True:
        ...
        match lang:
            ...
            case "eng":
                menu = GameMenu(english_words, fr.instraction_eng,
                                fr.welcome_frase_eng, fr.detail_eng,
                                fr.alphabet_eng, fr.to_play_eng, fr.word_eng,
                                fr.lose_eng, fr.win_eng, fr.error_eng)

            case "rus":
                menu = GameMenu(russian_words, fr.instraction_rus,
                                fr.welcome_frase_rus, fr.detail_rus,
                                fr.alphabet_rus, fr.to_play_rus, fr.word_rus,
                                fr.lose_rus, fr.win_rus, fr.error_rus)

            case _:
                print(fr.wrong_lang)
                continue

        # now all menu in english or in russia

        print(menu.welcome_frase)
        print(menu.detail)
        print(menu.play)

        # offer user instruction, exit or play again
                ...
                while game.attempt < 6 and game.win == 0:
                    ...
                    for i in range(game.num_letter):
                        if game.right_word[i] == enter_word[i]:
                            ...
                            menu.alphabet = color_string_letter(enter_word[i],
                                                                "green",
                                                                menu.alphabet)
                            continue
